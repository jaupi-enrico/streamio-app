...
# ── Build ─────────────────────────────────────────────────────
# Hostnames that must always be fetched through the server's media proxy, if
# this deployment has any (see needsSourceProxy in
# lib/features/watch/watch_providers.dart). Set STREAMIO_PROXY_HOSTS in
# scripts/release.env; unset means "decide from the scheme and the resolver's
# headers alone", which is what the web player does.
build_defines=()
if [[ -n "${STREAMIO_PROXY_HOSTS:-}" ]]; then
  build_defines+=(--dart-define=STREAMIO_PROXY_HOSTS="$STREAMIO_PROXY_HOSTS")
  echo "Pinned proxy hosts: ${STREAMIO_PROXY_HOSTS}"
fi
...
# ── Desktop packages ───────────────────────────────────────────
# Everything here is optional: only the APK is uploaded to the server, so a
# missing desktop toolchain must never abort a release that already built.
# The desktop artifacts exist to be attached to the GitHub release below.
release_assets=("$apk_path")

echo
read -r -p "Also build Linux desktop packages (.tar.gz/.deb) for this release? [y/N] " confirm_linux_pkg
if [[ "$confirm_linux_pkg" =~ ^[Yy]$ ]]; then
  if ./scripts/package-linux.sh --out=dist; then
    while IFS= read -r artifact; do
      release_assets+=("$artifact")
    done < <(find dist -maxdepth 1 -type f \
      \( -name "streamio-${new_semver}-linux-x64.tar.gz" -o -name "streamio_${new_semver}-${new_build}_amd64.deb" \) | sort)
  else
    echo "${YELLOW}warning:${RESET} Linux packaging failed — continuing without it."
    echo "  (Toolchain: clang cmake ninja pkg-config libgtk-3-dev; see docs/getting-started.md.)"
  fi
fi

# Windows can't be cross-compiled from here, so release.sh never builds it — it
# only picks up what scripts/package-windows.ps1 left in dist/ on a Windows box
# and was copied over. See docs/releasing.md.
windows_found=false
while IFS= read -r artifact; do
  release_assets+=("$artifact")
  windows_found=true
done < <(find dist -maxdepth 1 -type f -name "streamio-${new_semver}-windows-x64*" 2>/dev/null | sort)
if [[ "$windows_found" == true ]]; then
  echo "Found Windows artifacts in dist/ for ${new_semver} — they'll be attached to the GitHub release."
else
  echo "${YELLOW}No Windows artifacts${RESET} for ${new_semver} in dist/. To ship one, run"
  echo "  powershell -ExecutionPolicy Bypass -File scripts\\package-windows.ps1"
  echo "on a Windows machine, copy dist\\ over, and 'gh release upload v${new_version} <files>'."
fi
...
if [[ "$pushed" == true ]]; then
  ...
  if [[ "$confirm_gh_release" =~ ^[Yy]$ ]]; then
    ...
    gh release create "$tag" "${release_assets[@]}" \
      --title "${new_version}" \
      --notes "$notes" \
      || die "failed to create GitHub release ${tag}. Tag ${tag} was pushed; fix and re-run 'gh release create ${tag} ${release_assets[*]} --title ${new_version} --notes-file -' by hand."
