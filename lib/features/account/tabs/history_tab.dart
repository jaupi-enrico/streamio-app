...
/// account page has.
///
/// Pages like the library tabs. It used to ask for `limit: 100` in one shot,
/// which silently truncated anyone with a longer history — the server's own
/// cap is 100 per request, so there was no bigger number to ask for.
class HistoryTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    // "Clear history" only once the list has actually ended. Rendered under a
    // half-loaded list it reads as the bottom of the history, which it isn't.
    final hasClear = list.exhausted;
    ...
    return RefreshIndicator(
      ...
      child: PagedScrollLoader(
        ...
        child: ListView.separated(
          ...
          itemCount: list.items.length + 2 + (hasClear ? 1 : 0),
          ...
          itemBuilder: (context, i) {
            if (i == 0) {
              return PagedListCount(list: list, noun: 'entry');
            }
            final index = i - 1;
