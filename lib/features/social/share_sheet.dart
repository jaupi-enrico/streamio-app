...
/// How long a clip shared from the player runs. The web modal has a range
/// slider for both ends; the sheet only offers "start here", so the end is
/// derived.
const kDefaultClipSeconds = 60;
...
class _ShareSheetState extends ConsumerState<ShareSheet> {
  ...
  Future<void> _send() async {
    ...
    final clipStart = _asClip ? widget.suggestedClipStart : null;
    ...
    final ok = await runGuarded(
      ...
      () => ref.read(socialApiProvider).createShare(
            ...
            // `share.router.ts` rejects one bound without the other with a
            // 400, so "start at 12:30" has to carry an end. The web modal's
            // slider always produces both; here the end is the start plus a
            // default window.
            clipEndSeconds: clipStart == null
                ? null
                : clipStart + kDefaultClipSeconds,
