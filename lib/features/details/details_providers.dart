...
/// Every episode of a show in play order, flattened across season boundaries
/// (specials — season 0 — sort last). Empty for a movie.
///
/// Shared by [nextEpisodeProvider] and by the Chromecast payload, which sends
/// this list to the receiver so it can advance episodes on its own after the
/// app goes away.
final flatEpisodesProvider =
    FutureProvider.autoDispose.family<List<Episode>, ShowRef>((ref, args) async {
  final show = await ref
      .watch(showDetailsProvider((provider: args.provider, showId: args.showId)).future);
  if (show is! TvShow) return const [];
...
final nextEpisodeProvider =
  ...
  final allEpisodes = await ref.watch(
      flatEpisodesProvider((provider: args.provider, showId: args.showId)).future);
