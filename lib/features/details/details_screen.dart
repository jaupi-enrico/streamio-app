...
class _DetailsScreenState extends ConsumerState<DetailsScreen> {
  ...
  Future<void> _toggleWatchlist() => _requireSignIn(() async {
        ...
        final inList =
            ref.read(inWatchlistProvider(_showRef)).value ?? false;
  ...
  Future<void> _toggleFavorite() => _requireSignIn(() async {
        final api = ref.read(accountApiProvider);
        final isFavorite =
            ref.read(isFavoriteProvider(_showRef)).value ?? false;
        final ok = await runGuarded(
          context,
          () => isFavorite
  ...
  Future<void> _rate() => _requireSignIn(() async {
        final current = ref.read(myRatingProvider(_showRef)).value;
  ...
  Widget _movieBody(Movie movie) {
    ...
    final resumeAt = progressAsync.value?.progressSeconds ?? 0;
  ...
  Widget _actionRow(Show show) {
    final inWatchlist =
        ref.watch(inWatchlistProvider(_showRef)).value ?? false;
    final isFavorite =
        ref.watch(isFavoriteProvider(_showRef)).value ?? false;
    final myRating = ref.watch(myRatingProvider(_showRef)).value;
      ...
      children: [
        for (final genre in genres)
          ActionChip(
            ...
            // `go`, not `push`: /catalog lives inside the tabbed ShellRoute and
            // the shell is already below us in the stack. Pushing it would put
            // a second ShellRouteMatch in the root navigator — both carry the
            // same page key (the ShellRoute's hashCode), which trips
            // Navigator's duplicate-page-key assert.
            onPressed: () =>
                context.go('/catalog?genre=${Uri.encodeComponent(genre.id)}'),
          ),
      ],
    );
  }
                ...
                itemBuilder: (context, i) {
                  ...
                  return SizedBox(
                    ...
                    child: TvFocusable(
                      ...
                      //
                      // The provider this page was opened with, not
                      // `show.providerName` — see the note on
                      // `HomeScreen._openDetails`. A recommendation always
                      // comes from the same provider as its parent anyway.
                      onTap: () => context.replace(
                          '/details/${widget.provider}/${Uri.encodeComponent(show.id)}'),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Expanded(
