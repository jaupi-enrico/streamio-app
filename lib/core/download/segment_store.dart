...
class SegmentStore {
  SegmentStore({FlutterSecureStorage? storage})
      : _storage = storage ??
            // See TokenStore: flutter_secure_storage 10 dropped the deprecated
            // EncryptedSharedPreferences backend and migrates old entries on
            // first access. `migrateWithBackup` matters more here than anywhere
            // else — this key is the only thing that can decrypt already
            // downloaded segments, and a half-finished migration would render
            // every existing download unplayable.
