...
///
/// There is no compile-time base URL: the server is self-hosted and normally
/// reached through a DDNS name or the `redirect/` Cloudflare Tunnel, both of
/// which change per deployment. The user types it in on first launch (see
...
class ServerConfig {
  ...
  /// Accepts what a person actually types ("myhost.ddns.net",
  /// "http://192.168.1.10:3003/", "https://streamio.ddns.net/streamio") and
  ...
  /// commonly mounted under one (`https://streamio.ddns.net/streamio`), and
...
enum ServerProbeResult {
  ...
  /// Reachable, but not answering as Streamio. The `redirect/` tunnel serves
  /// a static `waiting.html` while `cloudflared` is still coming up, so a
  /// plain 200 of HTML is a real and expected state here — not a typo'd host.
  reachableNotReady,
...
Future<ServerProbe> probeServer(String baseUrl, {Dio? client}) async {
  ...
  try {
    ...
    if (status >= 200 && status < 300 && body.startsWith('{')) {
      // Adopt wherever the redirects landed. A DDNS name in front of the
      // `redirect/` tunnel answers with a 302 to the live tunnel URL, and
