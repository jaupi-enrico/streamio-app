...
/// Which stream a segment belongs to. Upstream output is commonly demuxed, so
/// a single download normally has both a `video` and an `audio` track, each
/// with its own segment list and its own generated playlist at playback time.
...
LazyDatabase _open() {
  return LazyDatabase(() async {
    ...
    // No `sqlite3_flutter_libs` and no old-Android workaround: package:sqlite3
    // 3.x ships SQLite itself through a Dart build hook, which is why that
    // plugin is now an empty end-of-life package.
