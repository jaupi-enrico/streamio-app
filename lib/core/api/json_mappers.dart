...
/// A `catalog` entry from `GET /api/providers` (`ProviderInfo` in
/// `core/core.ts`). A server that omits `displayName`/`description` for an
/// entry gets the same treatment as one that sent no catalog at all, so a
/// half-filled row never renders as a blank label.
///
/// `family`/`language`/`languages` are additive — an install that predates
/// provider families sends none of them, and the entry then stands alone as
/// its own single-language family.
ProviderInfo providerInfoFromJson(Map<String, dynamic> json) {
  final name = json['name']?.toString() ?? '';
  final fallback = ProviderInfo.fallback(name);

  return ProviderInfo(
    name: name,
    displayName: _str(json['displayName']) ?? fallback.displayName,
    description: _str(json['description']) ?? fallback.description,
    adult: json['adult'] == true,
    family: _str(json['family']),
    language: _str(json['language']) ?? '',
    languages: _objects(json['languages'])
        .map(_providerLanguageFromJson)
        .where((language) => language.slug.isNotEmpty)
        .toList(),
  );
}

ProviderLanguage _providerLanguageFromJson(Map<String, dynamic> json) {
  final code = _str(json['code']) ?? '';

  return ProviderLanguage(
    code: code,
    label: _str(json['label']) ?? languageLabelFor(code),
    slug: _str(json['slug']) ?? '',
  );
}
