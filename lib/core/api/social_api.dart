...
class SocialApi {
  ...
  /// Every count here is a Postgres `COUNT(*)`, i.e. a `bigint`, which
  /// node-postgres serialises as a string. These endpoints happen to
  /// `parseInt` before responding, but a cast that only holds because of a
  /// `parseInt` on the far side is one refactor from a `TypeError`.
  int _count(dynamic value) => switch (value) {
        num() => value.round(),
        String() => num.tryParse(value)?.round() ?? 0,
        _ => 0,
      };
  ...
  Future<({int followers, int following})> myFollowCounts() async {
    final json = await _client.get<Map<String, dynamic>>(
        ...
        authenticated: true);
    return (
      followers: _count(json['followers']),
      following: _count(json['following']),
    );
  }
