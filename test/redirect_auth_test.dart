...
///
/// A Streamio install is typically reached through a DDNS name that redirects
/// to an ephemeral Cloudflare Tunnel URL, so essentially every request is a
/// cross-host redirect. `dart:io`'s automatic redirect following **drops the
/// Authorization header** on that hop, so an authenticated request arrives
...
void main() {
  ...
  setUp(() async {
    ...
    // Stands in for the tunnel: records the request and answers.
    ...
    // Stands in for the DDNS host: redirects everything to the "tunnel",
