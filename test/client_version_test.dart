...
// `Override` moved out of the main entrypoint in Riverpod 3.
...
void main() {
  ...
  group('UpdateGate', () {
    ...
    // `MaterialApp.router`'s `builder`, which puts it *above* the Router —
    // there is no Navigator ancestor out there at all. A plain `MaterialApp`
    // with `home:` would not reproduce that, and would hide the bug this
    ...
    setUp(() {
      router = GoRouter(
        ...
        routes: [
          ...
          GoRoute(path: '/', builder: (_, __) => const Text('APP CONTENT')),
        ],
      );
    });
    ...
    testWidgets('shows the app when no update is pending', (tester) async {
      await tester.pumpWidget(wrap([
        ...
        clientOutdatedProvider.overrideWith(ClientOutdatedNotifier.new),
      ]));
    ...
    testWidgets('offers an optional update once, without blocking', (tester) async {
      await tester.pumpWidget(wrap([
        updateCheckProvider.overrideWith((ref) => advisory),
        clientOutdatedProvider.overrideWith(ClientOutdatedNotifier.new),
      ]));
      await tester.pumpAndSettle();
    ...
    // The regression this whole group exists for. The launch check lands while
    // the router is still on /splash; the bootstrap redirect to / then replaces
    // that page. A dialog pushed onto the router's navigator is a *pageless*
    // route attached to /splash and is discarded with it — the prompt appeared
    // and vanished a frame or two later. Drawn into UpdateGate's own subtree,
    // it survives.
    testWidgets('survives the router replacing the page underneath it',
        (tester) async {
      await tester.pumpWidget(wrap([
        updateCheckProvider.overrideWith((ref) => advisory),
        clientOutdatedProvider.overrideWith(ClientOutdatedNotifier.new),
      ...
      expect(find.text('Update available'), findsOneWidget,
          reason: 'the prompt must outlive the page it appeared over');
    });

    testWidgets('is dismissed by tapping outside it', (tester) async {
      await tester.pumpWidget(wrap([
        updateCheckProvider.overrideWith((ref) => advisory),
        clientOutdatedProvider.overrideWith(ClientOutdatedNotifier.new),
      ]));
      ...
      // Top-left corner: scrim, well clear of the centred dialog.
    ...
    testWidgets('replaces the whole app when the server is blocking it', (tester) async {
      await tester.pumpWidget(wrap([
        updateCheckProvider.overrideWith((ref) => null),
        // Overriding `build` rather than handing in a pre-loaded notifier: a
        // Notifier is rebuilt (and its state reset) whenever the API client is,
        // which here happens as soon as the unconfigured server settles.
        clientOutdatedProvider.overrideWithBuild((ref, notifier) =>
